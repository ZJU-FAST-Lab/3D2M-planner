...
void TrajOpt::addTimeIntPenalty(double& cost) {
  ...
  Eigen::Matrix<double, 6, 2> gradViolaOc;
  ...
  for (int i = 0; i < N; ++i) {
    ...
    for (int j = 0; j < innerLoop; ++j) {
      ...
      domega   = ( acc[1]*vel[0] - acc[0]*vel[1] ) / (vel.head(2).squaredNorm());
      ...
      // if (grad_cost_domega(domega, grad_tmp_1, cost_tmp)) {
        
      //   double vel_2 = vel.head(2).norm();
      //   double vel_4 = vel_2 * vel_2;
      //   double vel_4l = 1.0 / vel_4;
      //   double vel_2l = 1.0 / vel_2;
      //   Eigen::MatrixXd z_h3m   = acc.head(2).transpose() * B_h * vel.head(2);
      //   double z_h3 = z_h3m(0,0);
      //   z_h3m       = acc.head(2).transpose() * vel.head(2);
      //   double z_h1 = z_h3m(0,0);

      //   gradViolaOc = beta1 * (vel_2l * acc.head(2).transpose() * B_h - 2 * vel_2l * vel_2l * z_h3 * vel.head(2).transpose()) 
      //               + beta2 * vel_2l * vel.head(2).transpose() * B_h.transpose();
      //   gradViolaOc *= grad_tmp_1;


      //   gradViolaAt = alpha * grad_tmp_1 * ( vel_2l * (jer.head(2).transpose()*B_h*vel.head(2) - 2*vel_2l*z_h3*z_h1) );


      //   (this->jerkOpt).gdC.block<6, 2>(i * 6, 0) += omg * step * gradViolaOc;
      //   (this->jerkOpt).gdT(i) += omg * (cost_tmp / K + step * gradViolaAt);
      //   cost += omg * step * cost_tmp;
      // }
...
bool TrajOpt::grad_cost_p(const Eigen::Vector3d& p,
  ...
  Eigen::Vector3d gz(Eigen::Vector3d::Zero());
  ...
  // travel cost
  double a = ( 2*truncation ) / log(401);  //401 is (the max travel cost + 1)
  exp_travel_cost = (environment->travelcost_map) -> getSDFValue(p_drop);
  travel_cost = a * log(exp_travel_cost + 1);
  gtr         = -(environment->travelcost_map) -> getSDFGrad(p_drop);
  gtr         = a * gtr / (exp_travel_cost + 1);
  ...
  if( travel_cost > 0)
  {
    costp +=   0.3 * rhoP_tmp_ * pow( travel_cost,3 );
    gradp +=   0.3 * rhoP_tmp_ * 3 * pow( travel_cost,2 ) * gtr;
  }
  if(!environment->downproj)
  {
      double zdes = (environment->zcost_map) -> getDistWithGradTrilinear(p, gz);
      z_cost =  (p(2) - zdes) * (p(2) - zdes);
      if(z_cost > 0){
        // cout<<"costz = "<< z_cost<<"gz = "<<gz<<endl;
        costp +=  1 * rhoP_tmp_ * z_cost;
        gradp +=  1 * rhoP_tmp_ * 2 * (p(2) - zdes) * (Vector3d(0,0,1) - gz); 
      }  
  }
...
bool TrajOpt::grad_cost_domega( double domega,
                                double& graddomega,
                                double& costdomega) {
  
  double domegapen  = abs(domega) - 1.0;
  if (domegapen > 0) {
    graddomega = this->rhoV * 6 * domegapen * domegapen * domega;
    costdomega = this->rhoV * domegapen * domegapen * domegapen;
    return true;
  }
  return false;
}
...
//   v_norm(2) = 0;
//   v_norm.normalize();
//   Eigen::Vector3d v_verti;
//   v_verti(0) = -v_norm(1);
//   v_verti(1) =  v_norm(0);

//   Eigen::Vector3d plu, pru, pld, prd;

//   plu = p + 0.5*footprint_width*v_norm + 0.5*footprint_length*v_verti;
//   pru = p + 0.5*footprint_width*v_norm - 0.5*footprint_length*v_verti;
//   pld = p - 0.5*footprint_width*v_norm + 0.5*footprint_length*v_verti;
//   prd = p - 0.5*footprint_width*v_norm - 0.5*footprint_length*v_verti;

//   double tmp_cost_p;
//   Eigen::Vector3d tmp_grad_p;

//   if( grad_cost_p( plu, tmp_grad_p, tmp_cost_p ) )
//   {
//     costp += tmp_cost_p;
//     gradp += tmp_grad_p;
//   }

//   if( grad_cost_p( pld, tmp_grad_p, tmp_cost_p ) )
//   {
//     costp += tmp_cost_p;
//     gradp += tmp_grad_p;
//   }
  
//   if( grad_cost_p( pru, tmp_grad_p, tmp_cost_p ) )
//   {
//     costp += tmp_cost_p;
//     gradp += tmp_grad_p;
//   }
  
//   if( grad_cost_p( prd, tmp_grad_p, tmp_cost_p ) )
//   {
//     costp += tmp_cost_p;
//     gradp += tmp_grad_p;
//   }
...
bool TrajOpt::grad_cost_shape(const Eigen::Vector3d& p,const Eigen::Vector3d& v,
                          ...
                          double& costp,Eigen::Vector3d& gradv,double& costv) {
// std::cout<<"------------"<<std::endl;
  double footprint_length = 0.2;
  double footprint_width  = 0.2; 
  costp = 0;
  gradp = Eigen::Vector3d::Zero();
  costv = 0;
  gradv = Eigen::Vector3d::Zero();
...
if(gtr_for_v_3dim_tmp.norm()==0)
{
  continue;
    ...
    // Eigen::Matrix2d p0_derivate_multi_its_trans;p0_derivate_multi_its_trans<<n_y(0)*n_y(0),n_y(0)*n_y(1),n_y(1)*n_y(0),n_y(1)*n_y(1);
    // double p0_derivate_2_norm=sqrt(n_y(0)*n_y(0)+n_y(1)*n_y(1));
    // Eigen::Matrix2d x_axis_multi_its_trans;x_axis_multi_its_trans<<n_x(0)*n_x(0),n_x(0)*n_x(1),n_x(1)*n_x(0),n_x(1)*n_x(1);
    // double x_axis_2_norm=sqrt(n_x(0)*n_x(0)+n_x(1)*n_x(1));
